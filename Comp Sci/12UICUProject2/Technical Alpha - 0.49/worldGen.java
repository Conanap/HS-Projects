...
/**
 * Generates a world in a .txt file
 * 
 * @author Albion Fung 
 * @version 0.0.1b
 */
public class worldGen extends Actor
{
    ...
    private void saveInv()
    {
        ...
        invItems=player.getInventory().getItemArrayInfo();
        tbItems=player.getToolBar().getItemArrayInfo();
        boolean changeTheSave = false;
        String layer = "";
        try
        {
            ...
            {
                layer="";
                for(int j = 0; j < invItems[0].length; j++)
                {
                    if(invItems[i][j][0]!=99)
                        layer+=(i+"\n"+j+"\n"+invItems[i][j][0]+"\n"+invItems[i][j][1]+"\n"+invItems[i][j][2]);
                    else
                        layer+=(i+"\n"+j+"\n"+99+"\n"+99+"\n"+99);
                    if(i!=invItems.length-1)
                        layer+="\n";
                }
                WriteToFile(layer,changeTheSave,"invSave.txt");
                if(!changeTheSave)
                    changeTheSave=true;
            }
        }
        ...
        {
            System.out.println("Writing error (saveInv.inventory)");
        }
        ...
        try
        {
            for(int i = 0; i < tbItems.length; i++)
            {
                ...
                if(i!=invItems.length-1)
                    layer+="\n";
                WriteToFile(layer,changeTheSave,"tbSave.txt");
                if(!changeTheSave)
                    changeTheSave=true;
            }
        }
        ...
        {
            System.out.println("Writing error (saveInv.Toolbar)");
        }
    }
