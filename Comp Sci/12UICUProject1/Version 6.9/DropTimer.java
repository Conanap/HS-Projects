 ...
 * @0.0.3.OOP1
 */
public class DropTimer extends StatsDisplay
{
     ...
     */
    public DropTimer()//constructor to construct new health circles
    {
        bar = new GreenfootImage(70,70);//defining the greenfoot image called bar
        setImage(bar);
     ...
     */
    public DropTimer(double imHP)//constructor to construct new health circles with a specified maximum amount of hp
    {
        bar = new GreenfootImage(70,70);//defining the greenfoot image called bar
        setImage(bar);
    ...
    private void base ()//private method, creates the background and the base of the health bar
    {
        ...
        bar.fillOval(14,14,40,40);//drawing base of circle
    }
    ...
    public void update(boolean damage)//update method #1, taking boolean to determine if damage was done was used
    {
        ...
        diameter = 40*percent;//determining diameter of the inner circle; 80 is the diameter at 100% HP
        int radius =(int)(diameter/2);//determining radius in interger
        xcor=34-radius;//x coordinate of smaller circle, the background circle is 15 units from edge; to keep it centred, we subtract radius of smaller circle from the bigger circle(40) and add 15.
        ...
        if(currentHP<=0)
        {
            crate = new ResourceCrate();//when time is up, it creates a crate at its location
